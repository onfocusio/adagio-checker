...
export function displayAdServer() {
  ...
  if (typeof ADAGIO !== 'undefined') {
    ...
    if (adagioWindows) {
      ...
      if (unique.size) {
        const stringAdServer = Array.from(unique)
          .map((s) => `<code>${s}</code>`)
          .join(', ');
...
export function displayAdagioJs(prebidVersionDetected) {
  ...
  if (typeof ADAGIO !== 'undefined' && ADAGIO.versions && ADAGIO.versions.adagiojs)
    message = `Adagio.js: 🟢 <code>v${ADAGIO.versions.adagiojs}</code>`;
  else if (prebidVersionDetected >= 9)
    message = 'Adagio.js: 🔴 <code>Not loaded - Ensure RTD is setup.</code>';
