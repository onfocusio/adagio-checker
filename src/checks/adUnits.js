...
export function buildParamsCheckingArray(bid, paramsCheckingArray, apiRecordsItems) {
  ...
  const paramSite = bid?.params?.site;
  ...
  const ortb2ImpPlacement = bid?.ortb2Imp?.ext?.data?.placement;
  const ortb2ImpDivId = bid?.ortb2Imp?.ext?.data?.divId;

  // Check if there's a regs.ext.gdpr param (= 1) and a user.ext.consent param (consent string)
  const ortbGdpr = bid?.ortb2?.regs?.ext?.gdpr;
  const ortbConsent = bid?.ortb2?.user?.ext?.consent;

  // Since Prebid 9.39, Adagio supports interstitial and rewarded
  const ortb2ImpInterstitial = bid?.ortb2Imp?.instl;
  const ortb2ImpRewarded = bid?.ortb2Imp?.rwdd;
  ...
  const mediatypeBanner = bid.mediaTypes?.banner;
  const mediatypeVideo = bid.mediaTypes?.video;
  const mediatypeNative = bid.mediaTypes?.native;
  ...
  if (
    ...
    if (mediatypeBanner !== undefined) {
      const mediatypeBannerSizes = mediatypeBanner?.sizes;
    ...
    if (mediatypeVideo !== undefined) {
      ...
      const mediatypeVideoContext = mediatypeVideo?.context;
      const mediatypeVideoApi = mediatypeVideo?.api;
      const mediatypeVideoPlayerSize = mediatypeVideo?.playerSize;
      // Required for instream only
      const mediatypeVideoMimes = mediatypeVideo?.mimes;
      const mediatypeVideoPlcmt = mediatypeVideo?.plcmt || mediatypeVideo?.placement; // placement is deprecated, plcmt is the new one
      // Highly recommended for instream and outstream
      const mediatypeVideoPlaybackMethod = mediatypeVideo?.playbackmethod;
      // Highly recommended for instream only
      const mediatypeVideoStartDelay = mediatypeVideo?.startdelay;
      const mediatypeVideoStartProtocols = mediatypeVideo?.protocols;

      // For checking purpose
      const hasOutstreamContext = mediatypeVideoContext === 'outstream';
      const hasInstreamContext = mediatypeVideoContext === 'instream';
      const videoApiSupported = [1, 2, 3, 4, 5];
      const mimesExpected = ['video/mp4', 'video/ogg', 'video/webm', 'application/javascript'];
      const expectedInstreamPlcmt = 1;
      const protocolsExpected = [3, 6, 7, 8];
      ...
      if (mediatypeVideoApi !== undefined) {
        if (hasOutstreamContext && !mediatypeVideoApi.includes(2))
          ...
          !videoApiSupported.some((i) => mediatypeVideoApi.includes(i)) &&
          hasInstreamContext
        )
          paramsCheckingArray.push([
            chkr_badges.ko,
      ...
      if (
        ...
        mediatypeVideoPlayerSize.every(
          (subArr) => Array.isArray(subArr) && subArr.length === 2 && subArr.every(Number.isInteger)
        )
      ) {
        paramsCheckingArray.push([
          chkr_badges.ok,
    ...
    if (detectedCountryCodeIso3 === null) {
      ...
      if (EEACountries.includes(detectedCountryCodeIso3)) {
        ...
        const decodedConsent = ortbConsent !== undefined ? TCString.decode(ortbConsent) : null;
    ...
    if (ortb2ImpInterstitial !== undefined) {
      ...
      paramsCheckingArray.push([
        prebidVersionDetected < 9.39 ? chkr_badges.info : chkr_badges.info,
