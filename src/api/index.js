  ...
  if (bidReqOrgSitePairs.length) {
    ...
    for (const { organizationId, site } of bidReqOrgSitePairs) {
      ...
      if (orgIdApiDataResponse !== null) {
        ...
        const matchedDomainsRecords =
          orgIdApiDataResponse.records.filter((record) => hostname.includes(record.domain)) || null;
        const matchedSiteNameRecords =
          orgIdApiDataResponse.records.filter((record) => site === record.name) || null;
        const successRecordsItems =
          matchedDomainsRecords.filter((domainRecord) =>
            matchedSiteNameRecords.filter((siteNameRecord) => domainRecord === siteNameRecord)
          ) || [];
