 ...
 */
export function buildWrappersDropdownSelector(
  doc,
  prebidWrappers,
  prebidWrapper,
  switchToSelectedPrebidWrapperCallback
) {
...
export function createParametersCheckTable(doc, paramsCheckingArray, appendRowCallback) {
  ...
  for (const item of paramsCheckingArray) {
    const status = item[0];
 ...
 * @returns {void}
 */
export function createBidderParamsModal(
  doc,
  bid,
  paramsCheckingArray,
  bidderAdagioDetected,
  createParametersCheckTableCallback
) {
  ...
  if (bidderAdagioDetected) {
    ...
    const table = createParametersCheckTableCallback(doc, paramsCheckingArray, (tbody, s, p, d) =>
      appendParametersCheckerTableRow(doc, tbody, s, p, d)
    );
