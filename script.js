...
const ADAGIOSVG = Object.freeze({
    ...
    MANAGER:
        '<svg viewBox="0 0 24 24" style="height:1.2em;"><path d="M23 8c0 1.1-.9 2-2 2-.18 0-.35-.02-.51-.07l-3.56 3.55c.05.16.07.34.07.52 0 1.1-.9 2-2 2s-2-.9-2-2c0-.18.02-.36.07-.52l-2.55-2.55c-.16.05-.34.07-.52.07s-.36-.02-.52-.07l-4.55 4.56c.05.16.07.33.07.51 0 1.1-.9 2-2 2s-2-.9-2-2 .9-2 2-2c.18 0 .35.02.51.07l4.56-4.55C8.02 9.36 8 9.18 8 9c0-1.1.9-2 2-2s2 .9 2 2c0 .18-.02.36-.07.52l2.55 2.55c.16-.05.34-.07.52-.07s.36.02.52.07l3.55-3.56C19.02 8.35 19 8.18 19 8c0-1.1.9-2 2-2s2 .9 2 2z"></path></svg>',
    CHECKER:
    ...
    CONSENTS:
        '<svg viewBox="0 0 24 24" style="height:1.2em;"><path d="M13.17 4 18 8.83V20H6V4h7.17M14 2H6c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V8l-6-6zm-2 12c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zm4 3.43c0-.81-.48-1.53-1.22-1.85-.85-.37-1.79-.58-2.78-.58-.99 0-1.93.21-2.78.58C8.48 15.9 8 16.62 8 17.43V18h8v-.57z"></path></svg>',
...
function buildParamsCheckingArray(bid, paramsCheckingArray) {
    ...
    if (paramOrganizationId === undefined) paramsCheckingArray.push([STATUSBADGES.KO, `<code>params.organizationId</code>: <code>${paramOrganizationId}</code>`, `Parameter not found.`]);
    else {
        if (typeof paramOrganizationId === 'string' && !/^\d{4}$/.test(paramOrganizationId)) {
            paramsCheckingArray.push([STATUSBADGES.CHECK, `<code>params.organizationId</code>: <code>${paramOrganizationId}</code>`, `Should be a 4-digit integer or string (e.g., 1000 or '1000').`]);
        } else {
            paramsCheckingArray.push([STATUSBADGES.OK, `<code>params.organizationId</code>: <code>${paramOrganizationId}</code>`, ``]);
        }
    }
    ...
    if (divIdRes === undefined) paramsCheckingArray.push([divIdStatus, `<code>${divIdSetup}</code>: <code>${divIdRes}</code>`, `Not defined in the adUnit configuration.`]);
        ...
        if (divIdRes.trim() !== divIdRes) paramsCheckingArray.push([STATUSBADGES.CHECK, `<code>${divIdSetup}</code>: <code>${divIdRes}</code>`, `Space character detected.`]);
        else if (htlmDiv === null) paramsCheckingArray.push([STATUSBADGES.CHECK, `<code>${divIdSetup}</code>: <code>${divIdRes}</code>`, `Div id not found in the page.`]);
    ...
    else if (placementRes.trim() !== placementRes) paramsCheckingArray.push([STATUSBADGES.CHECK, `<code>${placementSetup}</code>: <code>${placementRes}</code>`, `Space character detected.`]);
    else if (/mobile/i.test(placementRes) || /desktop/i.test(placementRes) || /tablet/i.test(placementRes))
        paramsCheckingArray.push([STATUSBADGES.CHECK, `<code>${placementSetup}</code>: <code>${placementRes}</code>`, `Should not include reference to an environment`]);
    ...
    if (mediatypeBanner === undefined && mediatypeVideo === undefined && mediatypeNative === undefined)
        paramsCheckingArray.push([STATUSBADGES.KO, `<code>mediaTypes</code>: <code>${JSON.stringify(bid.mediaTypes)}</code>`, `No mediatype detected.`]);
    else {
        if (mediatypeBanner !== undefined) {
            ...
            if (mediatypeBannerSizes !== undefined) {
                ...
                if (commonArrays.length > 0) paramsCheckingArray.push([STATUSBADGES.OK, `<code>mediaTypes.banner.sizes</code>: <code>${JSON.stringify(commonArrays)}</code>`, ``]);
                else paramsCheckingArray.push([STATUSBADGES.KO, `<code>mediaTypes.banner.sizes</code>: <code>${JSON.stringify(mediatypeBannerSizes)}</code>`, `No supported size detected.`]);
            } else paramsCheckingArray.push([STATUSBADGES.KO, `<code>mediaTypes.banner.sizes</code>: <code>${JSON.stringify(mediatypeBannerSizes)}</code>`, `No parameter found.`]);
        }
        ...
        if (mediatypeVideo !== undefined) {
            ...
            if (hasOutstreamContext || hasInstreamContext) {
                paramsCheckingArray.push([STATUSBADGES.OK, `<code>mediaTypes.video.context</code>: <code>${mediatypeVideoContext}</code>`, ``]);
            } else {
                paramsCheckingArray.push([STATUSBADGES.KO, `<code>mediaTypes.video.context</code>: <code>${mediatypeVideoContext}</code>`, `No supported context found.`]);
            ...
            if (mediatypeVideoApi !== undefined) {
                if (hasOutstreamContext && !mediatypeVideoApi.includes(2))
                    paramsCheckingArray.push([STATUSBADGES.KO, `<code>mediaTypes.video.api</code>: <code>${JSON.stringify(mediatypeVideoApi)}</code>`, `Must support api <code>2</code>'`]);
                ...
                else paramsCheckingArray.push([STATUSBADGES.OK, `<code>mediaTypes.video.api</code>: <code>${JSON.stringify(mediatypeVideoApi)}</code>`, ``]);
            } else paramsCheckingArray.push([STATUSBADGES.KO, `<code>mediaTypes.video.api</code>: <code>${mediatypeVideoApi}</code>`, `No video api detected.`]);
            ...
            if (
                ...
                paramsCheckingArray.push([STATUSBADGES.OK, `<code>mediaTypes.video.playerSize</code>: <code>${JSON.stringify(mediatypeVideoPlayerSize)}</code>`, ``]);
            } else {
                paramsCheckingArray.push([STATUSBADGES.KO, `<code>mediaTypes.video.playerSize</code>: <code>${JSON.stringify(mediatypeVideoPlayerSize)}</code>`, `Wrong format or not size detected.`]);
            }
            ...
            if (hasInstreamContext) {
                if (mediatypeVideoMimes === undefined) {
                    paramsCheckingArray.push([STATUSBADGES.KO, `<code>mediaTypes.video.mimes</code>: <code>${JSON.stringify(mediatypeVideoMimes)}</code>`, `No mimes detected.`]);
                    ...
                    paramsCheckingArray.push([STATUSBADGES.OK, `<code>mediaTypes.video.mimes</code>: <code>${JSON.stringify(mediatypeVideoMimes)}</code>`, ``]);
                }
            }
            ...
            if (hasInstreamContext) {
                if (mediatypeVideoPlcmt === undefined || expectedInstreamPlcmt !== mediatypeVideoPlcmt) {
                    ...
                    paramsCheckingArray.push([STATUSBADGES.OK, `<code>mediaTypes.video.plcmt</code>: <code>${mediatypeVideoPlcmt}</code>`, ``]);
                }
            ...
            if (mediatypeVideoPlaybackMethod !== undefined) {
                ...
                if (expectedMethod && !mediatypeVideoPlaybackMethod.includes(expectedMethod)) {
                    ...
                    paramsCheckingArray.push([STATUSBADGES.OK, `<code>mediaTypes.video.playbackmethod</code>: <code>${JSON.stringify(mediatypeVideoPlaybackMethod)}</code>`, ``]);
                }
            } else {
                paramsCheckingArray.push([
                    STATUSBADGES.CHECK,
            ...
            if (hasInstreamContext) {
                if (mediatypeVideoStartDelay !== undefined) {
                    paramsCheckingArray.push([STATUSBADGES.OK, `<code>mediaTypes.video.startdelay</code>: <code>${mediatypeVideoStartDelay}</code>`, ``]);
                } else {
                    paramsCheckingArray.push([STATUSBADGES.CHECK, `<code>mediaTypes.video.startdelay</code>: <code>${mediatypeVideoStartDelay}</code>`, `No start delay detected.`]);
                }
            }
            ...
            if (hasInstreamContext) {
                if (mediatypeVideoStartProtocols !== undefined) {
                    if (protocolsExpected.every((i) => mediatypeVideoStartProtocols.includes(i))) {
                        paramsCheckingArray.push([STATUSBADGES.OK, `<code>mediaTypes.video.protocols</code>: <code>${mediatypeVideoStartProtocols}</code>`, ``]);
                    } else {
                        paramsCheckingArray.push([
                            STATUSBADGES.CHECK,
                    ...
                    paramsCheckingArray.push([STATUSBADGES.CHECK, `<code>mediaTypes.video.protocols</code>: <code>${mediatypeVideoStartProtocols}</code>`, `No protocol detected.`]);
                }
            }
        }
...
function checkAdServer() {
    ...
    if (stringAdServer === '') {
        appendCheckerRow(STATUSBADGES.CHECK, ADAGIOCHECK.ADSERVER, `No supported adserver: the viewability measurement may not work`);
    } else {
        appendCheckerRow(STATUSBADGES.OK, ADAGIOCHECK.ADSERVER, `${stringAdServer}`);
    }
}
...
function checkAdagioModule() {
    ...
    if (adagioAdapter !== undefined) {
        ...
        appendCheckerRow(adagioModuleStatus, ADAGIOCHECK.ADAPTER, adagiojsLog + wrapperIntegrityLog);
    } else {
...
function checkRealTimeDataProvider() {
    ...
    if (adagioAdapter !== undefined) {
        ...
        if (prebidInstalledModules !== undefined && prebidInstalledModules.length !== 0) {
            ...
            if (messageString !== '') {
                appendCheckerRow(computeBadgeToDisplay(true, 9, null), ADAGIOCHECK.RDTMODULE, messageString);
                return;
            }
        }
        ...
        if (prebidRtdModule.dataProviders !== undefined) {
            ...
            if (adagioRtdProvider !== null) {
                ...
                if (paramsOrgId === undefined)
                    appendCheckerRow(computeBadgeToDisplay(true, 9, null), ADAGIOCHECK.RDTMODULE, `Missing 'organizationId' parameter: <code>${JSON.stringify(adagioRtdProvider)}</code>`);
                else if (paramsSite === undefined)
                    appendCheckerRow(computeBadgeToDisplay(true, 9, null), ADAGIOCHECK.RDTMODULE, `Missing 'site' parameter: <code>${JSON.stringify(adagioRtdProvider)}</code>`);
                else if (!siteNames.includes(paramsSite) || !organizationIds.includes(paramsOrgId))
                    appendCheckerRow(computeBadgeToDisplay(true, 9, null), ADAGIOCHECK.RDTMODULE, `Parameters doesn't match with bids.params: <code>${JSON.stringify(adagioRtdProvider)}</code>`);
    ...
    if (prebidObject === undefined) {
        ...
        if (prebidOrtb2 !== undefined) {
            ...
            if (dataPagetype === undefined && dataCategory === undefined)
                appendCheckerRow(computeBadgeToDisplay('warn', 9, null), ADAGIOCHECK.ORTB2, `Missing 'pagetype'/'category': <code>ortb2.site.ext.data</code>`);
...
function checkUserIds() {
    ...
    if (presentUserIds.length > 0) {
        ...
        appendCheckerRow(STATUSBADGES.INFO, ADAGIOCHECK.USERIDS, 'No User IDs present');
    }
}
...
function checkSupplyChainObject() {
    ...
    if (prebidObject === undefined) {
        appendCheckerRow(STATUSBADGES.KO, ADAGIOCHECK.SCO, ADAGIOERRORS.PREBIDNOTFOUND);
        appendConsentsRow(STATUSBADGES.KO, ADAGIOCHECK.SCO, ADAGIOERRORS.PREBIDNOTFOUND);
        return;
    } else if (typeof prebidObject.getEvents !== 'function') {
        appendCheckerRow(STATUSBADGES.KO, ADAGIOCHECK.SCO, `<code>${prebidWrapper[0]}.getEvents()</code> is not a function`);
        appendConsentsRow(STATUSBADGES.KO, ADAGIOCHECK.SCO, `<code>${prebidWrapper[0]}.getEvents()</code> is not a function`);
        return;
    }
    ...
    if (adagioBid !== undefined) {
        appendCheckerRow(STATUSBADGES.OK, ADAGIOCHECK.SCO, `<code>${JSON.stringify(adagioBid.schain)}</code>`);
        // appendConsentsRow(STATUSBADGES.OK, 'Supply chain object', `<code>${JSON.stringify(adagioBid.schain)}</code>`);
    } else {
        appendCheckerRow(STATUSBADGES.CHECK, ADAGIOCHECK.SCO, 'If website is owned and operated, no SCO');
...
function checkConsentMetadata() {
    ...
    if (prebidObject === undefined) {
        ...
        appendCheckerRow(STATUSBADGES.CHECK, 'Consents', `<code>${prebidWrapper[0]}.getConsentMetadata()</code> not a function`);
        appendConsentsRow(STATUSBADGES.CHECK, 'Consents', `<code>${prebidWrapper[0]}.getConsentMetadata()</code> not a function`);
        return;
    }
    ...
    if (consentMetadata !== undefined) {
        ...
        } else appendCheckerRow(STATUSBADGES.CHECK, 'Consent metadata', `<code>${prebidWrapper[0]}.getConsentMetadata()</code>: <code>${JSON.stringify(consentMetadata)}</code>`);
    } else {
        appendCheckerRow(STATUSBADGES.CHECK, 'Consent metadata', `<code>${prebidWrapper[0]}.getConsentMetadata()</code>: <code>${JSON.stringify(consentMetadata)}</code>`);
    }
}
...
function checkGgprConsentString() {
    ...
    if (prebidBidsRequested !== undefined) {
        ...
        appendCheckerRow(STATUSBADGES.KO, 'GDPR consent string', `<code>${prebidBidsRequested}</code>`);
    }
}
