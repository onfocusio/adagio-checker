...
function createParametersCheckTable(paragraph, bid) {
    ...
    if (prebidVersion >= 9) {
        if (ortb2ImpDivId !== undefined) { divIdStatus = STATUSBADGES.OK; divIdSetup = 'ortb2Imp.ext.data.divId'; divIdRes = ortb2ImpDivId; }
        else if (paramAdUnitElementId !== undefined) { divIdStatus = STATUSBADGES.UPDATE; divIdSetup = 'params.adUnitElementId'; divIdRes = paramAdUnitElementId; }
        else { divIdStatus = STATUSBADGES.KO; divIdSetup = 'ortb2Imp.ext.data.divId'; divIdRes = undefined; }
    } else {
        if (paramAdUnitElementId !== undefined) { divIdStatus = STATUSBADGES.OK; divIdSetup = 'params.adUnitElementId'; divIdRes = paramAdUnitElementId; }
        else { divIdStatus = STATUSBADGES.KO; divIdSetup = 'params.adUnitElementId'; divIdRes = undefined; }
    }
    // AdUnitElementId (3/3): Then ensure the value is correct
    if (divIdRes === undefined) appendParametersCheckerTableRow(tbody, divIdStatus, `<code>${divIdSetup}</code>`, `Not defined in the adUnit configuration.`);
    else {
        const htlmDiv = document.getElementById(divIdRes);
        if (divIdRes.trim() !== divIdRes) appendParametersCheckerTableRow(tbody, STATUSBADGES.CHECK, `<code>${divIdSetup}</code>`, `Space character at the beginning or end of the string: <code>${divIdRes}</code>`);
        else if (htlmDiv === null) appendParametersCheckerTableRow(tbody, STATUSBADGES.CHECK, `<code>${divIdSetup}</code>`, `Id not found: <code>${divIdRes}</code>`);
        else appendParametersCheckerTableRow(tbody, divIdStatus, `<code>${divIdSetup}</code>`, `<code>${divIdRes}</code>`);
    }
    ...
    if (prebidVersion >= 9) {
        if (ortb2ImpPlacement !== undefined) { placementStatus = STATUSBADGES.OK; placementSetup = 'ortb2Imp.ext.data.placement'; placementRes = ortb2ImpPlacement; }
        else if (paramPlacement !== undefined) { placementStatus = STATUSBADGES.UPDATE; placementSetup = 'params.placement'; placementRes = paramPlacement; }
        else { placementStatus = STATUSBADGES.KO; placementSetup = 'ortb2Imp.placement'; placementRes = undefined; }
    } else {
        if (paramPlacement !== undefined) { placementStatus = STATUSBADGES.OK; placementSetup = 'params.placement'; placementRes = paramPlacement; }
        else { placementStatus = STATUSBADGES.KO; placementSetup = 'params.placement'; placementRes = undefined; }
    }
    // Placement (3/3): Then ensure the value is correct
    if (placementRes === undefined) appendParametersCheckerTableRow(tbody, placementStatus, `<code>${placementSetup}</code>`, `Not defined in the adUnit configuration.`);
    else if (placementRes.trim() !== placementRes) appendParametersCheckerTableRow(tbody, STATUSBADGES.CHECK, `<code>${placementSetup}</code>`, `Space character at the beginning or end of the string: <code>${placementRes}</code>`);
    else if (/mobile/i.test(placementRes) || /desktop/i.test(placementRes) || /tablet/i.test(placementRes)) appendParametersCheckerTableRow(tbody, STATUSBADGES.CHECK, `<code>${placementSetup}</code>`, `Includes reference to an environment: <code>${placementRes}</code>`);
    else appendParametersCheckerTableRow(tbody, placementStatus, `<code>${placementSetup}</code>`, `<code>${placementRes}</code>`);
...
function checkAdServer() {
    ...
    const tabName = ADAGIOTABSNAME.CHECKER.toLowerCase().replace(' ', '-');
    const alertTextDiv = overlayFrameDoc.getElementById(`${tabName}-alert`);
    if (stringAdServer !== "") alertTextDiv.innerHTML += `<small>• Adserver(s) detected: ${stringAdServer}</small><br>`;
...
function checkDeviceAccess() {
    ...
    if (prebidObject === undefined) {
        ...
        appendCheckerRow(computeBadgeToDisplay(deviceAccess ? false : true, 9, null), ADAGIOCHECK.DEVICEACCESS, `<code>${prebidWrapper[0]}.getConfig('deviceAccess')</code>: <code>deviceAccess</code>`);
    }
}
    ...
    if (prebidObject != undefined) {
        if (adagioAdapter !== undefined) {
            ...
            if (!hasEligibleVersion) {
                appendCheckerRow(STATUSBADGES.INFO, ADAGIOCHECK.ANALYTICS, `Prebid version too low: <code>${prebidVersion}</code>`);
            } else if (!hasEnabledAnalytics) {
                appendCheckerRow(STATUSBADGES.INFO, ADAGIOCHECK.ANALYTICS, `Analytics not enabled: <code>${hasEnabledAnalytics}</code>`);
            } else {
                appendCheckerRow(STATUSBADGES.OK, ADAGIOCHECK.ANALYTICS, `Prebid version: <code>${prebidVersion}</code> / Analytics: <code>${adagioAdapter.versions?.adagioAnalyticsAdapter}</code>`);
            }
        } else {
            appendCheckerRow(STATUSBADGES.KO, ADAGIOCHECK.ANALYTICS, `No Adagio bidder adapter found: <code>${adagioAdapter}</code>`);
        }
    } else {
...
function checkUserIds() {
    ...
    if (presentUserIds.length > 0) {
        ...
        appendCheckerRow(STATUSBADGES.OK, ADAGIOCHECK.USERIDS, `
...
function checkConsentMetadata() {
    ...
    if (consentMetadata !== undefined) {
        ...
        } else appendCheckerRow(STATUSBADGES.KO, 'Consent metadata', `<code>${prebidWrapper[0]}.getConsentMetadata()</code>: <code>${consentMetadata}</code>`);
    } else {
        appendCheckerRow(STATUSBADGES.KO, 'Consent metadata', `<code>${prebidWrapper[0]}.getConsentMetadata()</code>: <code>${consentMetadata}</code>`);
    }
}
